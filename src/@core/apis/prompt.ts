...
export async function updatePrompt(id: string, data: Partial<PromptFormData>): Promise<PromptResponse> {
...
export async function updatePromptFiles(
  ...
  const response = await apiClient.patch<PromptFilesResponse>(`/cms/prompts/${id}/files`, formData, {
